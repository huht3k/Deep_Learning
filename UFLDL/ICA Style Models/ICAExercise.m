...
% % 
% % numGrad = computeNumericalGradient2( @(x) orthonormalICACost(x, visibleSize, numFeatures, patches, epsilon), weightMatrix );
...
%         error('Projection seems okay. Comment out verification code before running optimization.');
    ...
    if mod(iteration, 1000) == 0
        ...
        hdl = figure(floor(iteration / 1000));
        nFig = floor(iteration / 1000);
        displayColorNetwork(weightMatrix'); 
        
        namePart1 = 'weightMatrix';
        namePart2 = strcat(num2str(nFig), '.jpg');
        nameFig = strcat(namePart1, namePart2);
        print(hdl, '-djpeg', nameFig); 
%         print -djpeg nameFig;
    end
