...
%                   otherwise.
    ...
    % -------------------- YOUR CODE HERE --------------------   
    grad = 2 * (weightMatrix * featureMatrix - patches) * featureMatrix'/ numExamples ...
        + 2 * gamma * weightMatrix;
    grad = grad(:);
    cost = sum(sum((weightMatrix * featureMatrix - patches) .^ 2)) / numExamples + gamma * sum(sum(weightMatrix .^ 2));
